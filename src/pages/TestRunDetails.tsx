      ...
      <div className="min-h-screen bg-slate-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16 items-center">
              ...
              <button
                ...
                className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${colors.danger.bg} hover:${colors.danger.bgHover}`}
              >
                Sign Out
              </button>
            </div>
        ...
        <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          <div className="text-center py-12">
            <p className="text-slate-500">Loading test run details...</p>
          </div>
        </main>
      </div>
    );
  ...
  if (error || !testRun) {
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          <div className={`${colors.danger.bgLight} border ${colors.danger.border} ${colors.danger.textDark} px-4 py-3 rounded`}>
            {error || 'Test run not found'}
          </div>
          <button
            onClick={() => navigate('/test-runs')}
            className="mt-4 px-4 py-2 border border-slate-300 rounded-md shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50"
          >
            Back to Test Runs
          </button>
        </main>
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="mb-6">
            <button
              onClick={() => navigate('/test-runs')}
              className="text-stone-600 hover:text-stone-900 text-sm font-medium mb-4"
            >
              ← Back to Test Runs
            </button>
            <div>
              <h1 className="text-3xl font-bold text-slate-900">{testRun.name}</h1>
              <p className="mt-2 text-sm text-slate-600">{testRun.description}</p>
                ...
                <div className="divide-y divide-slate-200 max-h-96 overflow-y-auto">
                  {testCases.length === 0 ? (
                    <div className="px-4 py-8 text-center">
                      <p className="text-sm text-slate-500">No test cases in this run</p>
                    </div>
                  ) : (
                    testCases.map((testCase) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 ${colors.background.hover} focus:outline-none focus:ring-2 focus:ring-inset focus:ring-stone-500 transition-colors ${
                          selectedTestCaseId === testCase.id ? `${colors.background.selected} border-l-4 ${colors.background.selectedBorder}` : ''
                        }`}
                      >
                        <div className="flex items-start justify-between">
                          ...
                          {testCase.execution && (
                            <span
                              className={`flex-shrink-0 ml-2 px-2 py-1 text-xs leading-4 font-semibold rounded ${colorHelpers.getTestResultBadge(testCase.execution.status)}`}
                            >
            ...
            <div className="flex-1">
              {selectedTestCase ? (
                <div className="bg-white shadow rounded-lg overflow-hidden">
                  <div className="p-6 border-b border-slate-200">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`inline-block ${colors.primary.badge} px-3 py-1 rounded-full text-xs font-medium`}>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-2">Preconditions</h3>
                        <p className="text-sm text-slate-600 whitespace-pre-wrap">
                          {selectedTestCase.preconditions}
                        </p>
                      </div>
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-2">Test Steps</h3>
                        <p className="text-sm text-slate-600 whitespace-pre-wrap">
                          {selectedTestCase.testSteps}
                        </p>
                      </div>
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-2">Expected Result</h3>
                        <p className="text-sm text-slate-600 whitespace-pre-wrap">
                    ...
                    <div className="border-t pt-6">
                      <h3 className="text-lg font-semibold text-slate-900 mb-4">Test Execution</h3>
                      ...
                      <div className="mb-4">
                        ...
                        <textarea
                          ...
                          className={`w-full px-3 py-2 border ${colors.form.input} rounded-md shadow-sm focus:outline-none ${colors.form.inputFocus} text-sm`}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-slate-200">
                        <div>
                          ...
                          <input
                            ...
                            value={user?.email || ''}
                            disabled
                            className={`w-full px-3 py-2 border ${colors.form.input} rounded-md ${colors.form.disabled} text-sm`}
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-700 mb-2">
                ...
                <div className="bg-white shadow rounded-lg p-6 text-center">
                  <p className="text-slate-500">Select a test case to view details</p>
                </div>
              )}
            </div>
          </div>
